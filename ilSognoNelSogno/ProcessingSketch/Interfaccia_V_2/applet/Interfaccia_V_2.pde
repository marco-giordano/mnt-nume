...
if (tobj.getSymbolID() == 0){ // controllo dell'ID del fiducial
     stroke(int(map(tobj.getScreenX(width), 0, width, 0, 255))); // colore linee orizontali che varia con la posizione
   ...
   stroke(int(map(tobj.getScreenY(height), 0, height, 0, 255)));  // colore linee verticali che viariano con la posizione
